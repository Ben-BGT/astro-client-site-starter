...
const heroBlock = fields.object({
  variant: fields.select({
    label: "Hero variant",
    description: "Left-aligned: default workhorse. Centered: balanced, good for landing pages. Split with image: text left, photo right. Dark bold: inverted statement hero.",
    options: [
  ...
  image: fields.image({
    label: "Image",
    description: 'Only shown by the "Split with image" variant. Ignored otherwise.',
...
const testimonialWallBlock = fields.object({
  ...
  source: fields.select({
    label: "Source",
    description: 'Featured: only testimonials with the "featured" flag. All: everything in the testimonials collection.',
    options: [
...
const statsBlock = fields.object({
  ...
  items: fields.array(
    ...
    {
      label: "Items",
      itemLabel: (props) => `${props.fields.value.value || ""} ${props.fields.label.value || ""}`.trim() || "Stat"
    }
  )
});
...
const contentBlockBlock = fields.object({
  body: fields.text({
    label: "Body",
    description: "Long-form text. Separate paragraphs with a blank line. Renders inside a narrow, readable column.",
    multiline: true,
    defaultValue: ""
  })
});
        ...
        design: fields.object(
          {
            themePreset: fields.select({
              label: "Theme preset",
              description: "Starting point for colours, fonts and radius. The overrides below win over whatever the preset ships with.",
              options: [
            ...
            primaryColor: fields.text({
              ...
              description: "Optional. Overrides the preset primary. Example: #2563EB. Leave blank to keep the preset default.",
            ...
            fontBody: fields.text({
              ...
              description: 'CSS font-family value. Example: "Inter, system-ui, sans-serif". Remember to set the Google Fonts URL below if you reference a web font.'
            }),
            ...
            googleFontsHref: fields.url({
              ...
              description: "Paste the full import URL from fonts.google.com, e.g. https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&display=swap"
            }),
            ...
            headerVariant: fields.select({
              ...
              description: "Minimal: name left, nav right. Centered: name above nav. Split: nav flanking centered name.",
              options: [
            ...
            footerVariant: fields.select({
              ...
              description: "Simple: one-line. Columns: three columns of links and contact. Minimal: copyright only.",
              options: [
          ...
          {
            label: "Design",
            description: "Colors, fonts, radius, and Header/Footer variants. Pick a preset for a starting point, then fine-tune. Hero style is now a block-level choice on each page."
          }
        )
      }
    }),
    ...
    landingPages: collection({
      ...
      schema: {
        ...
        showChrome: fields.checkbox({
          ...
          description: "Uncheck for a pure, chrome-free landing page (no nav, no footer). Handy for paid-ads landing pages.",
          defaultValue: true
        }),
        blocks: pageBlocks("Page blocks")
      }
    ...
    caseStudies: collection({
      ...
      schema: {
        ...
        metrics: fields.array(
          ...
          {
            label: "Metrics",
            itemLabel: (props) => `${props.fields.label.value || "Metric"}: ${props.fields.value.value || ""}`
          }
        ),
        cover: fields.image({
          label: "Cover image",
